...
namespace N_arryTreeDataStructure
{
    public class Helper
    {
        ...
        public void PrintTreeNodes(List<TreeNode> treeNodes)
        {
            string outStr;
            ...
            if(treeNodes != null)
            {
                ...
                {
                    outStr = string.Empty;
                    if (treeNodeItem.ItemLevel > 0)
                    {
                        for (int i = 0; i < treeNodeItem.ItemLevel; i++)
                        {
                            outStr += "\t";
                        }
                    }

                    Console.WriteLine($"{outStr}{treeNodeItem.MenuName}");

                    if(treeNodeItem.ChildrenNodes != null)
                    {
                        PrintTreeNodes(treeNodeItem.ChildrenNodes);
                    }
                }
            }
        }
